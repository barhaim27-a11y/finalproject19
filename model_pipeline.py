...
def train_model(data_path: str, model_name: str, model_params: dict,
    ...
    if do_cv:
        ...
        import numpy as _np
        try:
            scores = cross_validate(pipe, X_tr, y_tr, cv=cv, scoring=scoring, n_jobs=-1, return_train_score=False, error_score=_np.nan)
            cv_means = {m: float(_np.nanmean(scores[f"test_{m}"])) for m in ["roc_auc","accuracy","f1","precision","recall"]}
