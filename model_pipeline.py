...
def train_model(data_path: str, model_name: str, model_params: dict,
    ...
    if do_cv:
        try:
            ...
            cv_scores = cross_validate(
                ...
                return_train_score=False, error_score=_np.nan
            )
            cv_means = {
                m: float(_np.nanmean(cv_scores.get(f"test_{m}", _np.array([_np.nan]))))
                for m in ["roc_auc","accuracy","f1","precision","recall"]
            }
        except Exception as _e:
            cv_means = None  # continue without CV rather than crash
    ...
    if do_tune:
        try:
            ...
            # If tuning fails, keep the original pipe
            pass
